...
*/
		...
		$obj = new miscelaneos('','','','','');
		$obj->eliminar_all_miscelaneos('bannerInterclubes');
		...
		foreach($datosBanner as $datosBanner_key => $datosBanner_info){
			...
			$obj = new miscelaneos($_POST['miscelaneo'],'bannerInterclubes',utf8_decode($datosBanner_info[0]),
									'',$imagen);
		...
		$obj = new miscelaneos('','','','','');
		$banners = $obj->get_all_banner_interclubes();
